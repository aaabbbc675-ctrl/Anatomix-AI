...
(async () => {
  ...
  check("FITNESS_BODYBUILDING — این هم سناریوی fat_raised_to_generic_floor واقعی است (weight=70, target=1700)", () => {
    // چرا این ردیف تست جداگانه‌ی «تمیز/بدون‌هشدار» ندارد (برخلاف بقیه‌ی ۷
    // ردیف پایین‌تر در همین فایل): با جبر ثابت می‌شود که برای این ردیف
    // (protein=1.6, fat=0.5, carb منطقی=[3,5]) هیچ ترکیب وزن/کالری‌ای وجود
    // ندارد که هم fat_raised_to_generic_floor را نزند هم carb را داخل بازه
    // نگه دارد — دو شرط جبراً ناسازگارند:
    //   شرطِ «کف عمومی نزند» (fat رشته‌ای غالب بماند): T ≤ 22.5w
    //   شرطِ «carb داخل [3,5] بماند»:                    T ∈ [22.9w, 30.9w]
    // چون 22.5w < 22.9w، این دو بازه هرگز هم‌پوشانی ندارند — پس سناریوی
    // «هر دو راضی» برای این ردیف با این اعداد اصلاً وجود خارجی ندارد؛ به‌جای
    // ساختن یک سناریوی مصنوعی جدا برای fat_raised_to_generic_floor، همین
    // تست هر دو نقش را هم‌زمان ایفا می‌کند.
    //
